...
# Comparing Lebron and Jordan's Player Performance Analysis
```{r}
convert_percentage <- function(perc) {
  as.numeric(sub("%", "", perc))
}
...
CombinedDataLonger %>%
  ...
  theme_minimal() +
  facet_wrap(~Statistic, scales = "free_y") +
  labs(title = "Player Performance Analysis", 
...
# Comparing Lebron and Jordan's Personal Efficiency Rating At Ages 30+ Each Season
```{r}
OldBron <- LebronPer100Poss %>%
  filter(Season > "2014-15") %>%
  mutate(PER = (PTS + TRB + AST + STL + BLK - ((FGA - FG) + (FTA - FT) + TOV)) / G, 
